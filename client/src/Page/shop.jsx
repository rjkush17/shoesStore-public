    ...
    <>
      <div className="w-screen overflow-x-hidden">
        ...
        <div className="flex flex-col-reverse tablet:flex-row w-full">
              {sorted.length > 0 ? (
                <section className="mx-auto w-full mobile:w-10/12 grid gap-0 mobile:gap-8 grid-cols-1 mobile:grid-cols-2 grid:grid-cols-3 tablet:grid-cols-3">
               { sorted.map((val, ind) => <ProductCard key={ind} val={val} />)}
                </section>
              ) : (
                <p className="w-full text-center h-96 flex justify-center items-center">No products found within the specified price/Category.</p>
              )}
          <section className="leading-8 w-full tablet:w-[20%] pl-6 mobile:ml-0 text-lg text-gray-500">
            <div className="mt-8">
              <h2 className="font-bold underline mb-3 text-black">
                Categories
              </h2>
              <p className="" onClick={() => navigate("/shop/all")}>
                All type shoes
              </p>
            ...
            <div className="mt-8">
              ...
              <p onClick={() => handleSortByPrice("asc")}>Price Low to High</p>
              <p onClick={() => handleSortByPrice("dsc")}>Price High to low</p>
            </div>
            <div className="mt-8">
              <h2 className="font-bold underline mb-3 text-black">
                Fliter by Price
